...
export const getQuantityOfRejectedPromises = (promises) => {
  ...
  return Promise.allSettled(promises)
    .then((resultsArr) => resultsArr.filter((object) => object.status === "rejected").length);
};

export const getQuantityOfFulfilledPromises = (promises) => {
  //*  write code to pass test ⬇ ️
  return Promise.allSettled(promises)
    .then((resultsArr) => resultsArr.filter((object) => object.status === "fulfilled").length);
};
...
export const fetchAllCharactersByIds = async (ids) => {
  ...
  //*  write code to pass test ⬇ ️
  const arrPromises = ids.map((id) => fetchCharacterById(id));
